...
class ProfilerCallbackHandler :
	public ICorProfilerCallback7 {
private:
	std::atomic<int> mRefCount;
	ICorProfilerInfo7 *mProfilerInfo;
	boost::asio::io_service mIoSvc;
	// Create io_service::work object to stop io_service.run() from returning early which
	// prevents async callbacks from being called (e.g async_write_some)
	boost::asio::io_service::work mIoWork;
	boost::thread mWorkThread;
	HostCommClient mCommClient;
	std::shared_ptr<MetadataHelper> mMdHelper;
	TraceSettings mTraceSettings;

	void releaseProfilerInfo();

public:
	ProfilerCallbackHandler();
	virtual ~ProfilerCallbackHandler();
	...
	// ICorProfilerCallback8 interface required for these
